...
First we'll talk about logging, and how you can use it to better understand the process by which vcr matches the requests you maketo the request-response pairs saved in a cassette.
...
Lets us logging to look at what happens the first time you make a request:
...
First a new cassette is inserted with recording turned on.
...
Now when vcr inserts the cassette, it loads one interaction from disk and is in replaying mode, rather than recording.
Then it matches the request to one that was saved earlier and replays the response.
