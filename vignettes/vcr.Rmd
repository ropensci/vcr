...
It's good practice to look at the cassette files that are saved to disk. This will help you understand how vcr works by seeing exactly what data it has to work with. It will also come in handy if you ever need to edit the cassettes. Manually editing is typically most useful when you're testing error handling code, because it allows you to create responses that would otherwise be hard to get the API to generate.
...
* _interaction_`: a HTTP request and HTTP response pair. When a request is replayed, vcr finds a matching request on disk, and uses that save response.
