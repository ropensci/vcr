...
It makes your tests independent of your internet connection and because your tests get much much faster, you can write even more, increasing the coverage of your package.
...
This lets us make real HTTP requests without having to worrying about whether or not the internet is working.
...
## What happens in the API changes?
...
-   Include a small number of tests that don't cassettes, so that they always use the live API.
...
-   Set up a GitHub action (or similar) that runs `R CMD check` with env var `VCR_TURN_OFF=true`.
    This turns of all vcr usage so that all requests are live.
...
Now that you're familiar using vcr for your unit tests, you might wonder if there are other places you can use it in your packages.
There are!
...
-   Use `vcr::insert_example_cassette()` and `vcr::eject_casssette()` in examples.
